...
- 报告**落盘**：`docs/<模块名>V<版本号>-<YYYYMMDDHHMMSS>/5.2-<功能名>-规范核对报告.md`（走 ai-dev-workflow 流程时；**独立触发无 docs 目录时** → 直接在对话里输出报告 + 提示用户是否落盘）；12 项 HIGH + C/N 组 + INFO + 场景化结论汇总进**验收报告**「关键规范落地核对表」
...
- [ ] 核对报告已输出（走流程时落盘 `docs/<模块名>V<版本号>-<YYYYMMDDHHMMSS>/5.2-<功能名>-规范核对报告.md`）
